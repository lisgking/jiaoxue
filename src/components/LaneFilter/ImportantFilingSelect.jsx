...
export default class ImportantFilingSelect extends FilterSuper {
  constructor(props) {
    super(props);
  ...
  fetchFilterData = (keyword) => {
    ...
    ask('filterSelection', { params: { laneTemplateId: 1019, selectionType: 4 } }).then((res) => {
      if (res.code === 200) {
        const { tagKey, valueKey, valueFn } = this.props;
        const options = transToTree(res.data, { tagKey: 'text', valueKey: 'value', valueFn });
        console.log(options);
        this.setState({
          loading: false,
          loaded: true,
